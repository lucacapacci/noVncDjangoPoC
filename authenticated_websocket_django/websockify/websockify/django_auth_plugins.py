...
def get_host_port(current_user):
    ...
    current_user_host_port = UserHostPort.objects.filter(username=current_user.username)
    if len(current_user_host_port) == 1:
        return (current_user_host_port[0].host, current_user_host_port[0].port)
    else:
        raise AuthenticationError(response_code=403)
